...
module Ai
  class OpenaiHttpClient
    ...
    # Strategist JSON can be large; default 5m. Override with OPENAI_HTTP_READ_TIMEOUT.
    ...
    # Enough room for long bullet_rewrites + checklist JSON (gpt-4* chat completions).
    ...
    def self.chat_json(messages:, model:)
      ...
      begin
        ...
        raise ApiError.new(
          "OpenAI request timed out (#{e.class}: #{e.message}). Increase OPENAI_HTTP_READ_TIMEOUT (e.g. 420) or use a faster model.",
          status_code: nil,
          body: nil
        )
      rescue ApiError => e
    ...
    def self.perform_request(uri, api_key, payload)
      ...
      http.write_timeout = DEFAULT_OPEN_TIMEOUT if http.respond_to?(:write_timeout=)
    ...
    def self.handle_response!(response)
      ...
      when 429, 500, 502, 503, 504
        raise ApiError.new("OpenAI request failed (retryable)", status_code: response.code.to_i, body: response.body)
      else
